...
export default function SportDetails() {
  ...
  useEffect(() => {
    const stored = sessionStorage.getItem("selectedCard")
    if (stored) {
  ...
  const { sport, image, description, time, date, location, peoplemissing, author } = card
      ...
      <div className="w-full max-w-7xl flex flex-col items-center gap-10">
        <div className="w-full max-w-2xl bg-gray-900 rounded-2xl overflow-hidden shadow-2xl border-2 border-black">
          <div
            ...
            style={{ backgroundImage: `url(${image || "/default-image.jpg"})` }}
          />
          <div className="p-6 sm:p-8">
            ...
            <div className="mt-6 space-y-2 text-lg text-gray-400">
              ...
              <p><span className="font-semibold text-white">⏰ Time:</span> {time}</p>
              <p><span className="font-semibold text-white">👥 Missing players:</span> {peoplemissing}</p>
              <p><span className="font-semibold text-white">✍️ Author:</span> {author}</p> 
            </div>
            ...
            <div className="mt-8 flex justify-center">
              <button
                ...
                onClick={() => alert(`${author}'s contact is +387 63 123 456`)}
              >
                Contact
              </button>
            </div>
